...
def Rnoise(orignal):
    img = cv2.imread(orignal)
    #由于opencv不支持读取中文路径，用以下方法代替cv2.imread
    img = cv2.imdecode(np.fromfile(orignal, dtype=np.uint8), 1)
    ...
    #图片反色
    thresh_img = cv2.bitwise_not(h_erode_img)
    cv2.imshow("inverse",thresh_img)

    #cv2.waitKey(0)
    cv2.destroyAllWindows()
    img3 = Image.fromarray(thresh_img)
    img3.save(orignal)

if __name__=="__main__":
    Rnoise(r"D:\hewei\soft\check report2\mistakepic\1519729313.jpg")
